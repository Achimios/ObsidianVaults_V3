...
- ✅ **[2026-04-02]** Git 历史全清：V1/V2/V3 孤儿重置 + push --force，各 1 条历史
- ✅ **[2026-04-02]** mlflow 研究报告 + 可执行方案（含用户纠正版）→ `-人类思维笔记/Y_从他人的AI工作流学习/github_mlflow/`
- ✅ **[2026-04-02]** AGENTS.md × 4：magic 从 `<!-- -->` 改为 `<magic>` XML tags（Claude Code strip 注释而非 XML tags）
- ✅ **[2026-04-02]** AGENTS.md root：延伸法则段从常驻移进 `-project-continue` Skill，节省常驻 token
- ✅ **[2026-04-02]** `make_claude_symlinks.bat`：batch glob AGENTS.md 硬链接为 CLAUDE.md（硬链接无需 admin）
- ✅ **[2026-04-02]** 4个 CLAUDE.md 硬链接部署：根目录 / Python / 架构演示 / 人类思维笔记
- ✅ **[2026-04-02]** `.claude/skills` junction 链接 → `.github/skills`（Claude Code 共用 Skills）
- ✅ **[2026-04-02]** `.gitignore` 加入 `**/CLAUDE.md` + `.claude/skills/`
- ✅ **[2026-04-02]** Skills `-project-continue` 建立：分层读取 + -CONTEXT格式 + -LEARNINGS + -where_continue
- ✅ **[2026-04-02]** Skills `🎮义体-tts` 建立：TTS 守护进程接口 + 声线隔离铁律
- ✅ **[2026-04-02]** Skills 研究报告 saved → ai_Copilot_SKILL/
- ✅ **[2026-04-02]** AI 框架接入点表 AGENTS.md 更新：CLAUDE.md 标注硬链接来源
- ✅ **[2026-04-02]** 泰戈尔 AGENTS.md 加浪漫哲学 magic；架构演示 AGENTS.md 加 magic
- ✅ **[2026-04-02]** Instructions 文件改名：三个域加 `-` 前缀，Python 加 `@` 前缀；`明日香.agent.md` 加 `🎮` 前缀；所有引用已更新
- ✅ **[2026-04-02]** Claude Code 机制研究两文件建立（z_ClaudeCode机制研究 + 可执行操作）
- ✅ **[2026-04-02]** `@python.instructions.md` 占位建立（applyTo: **/*.py）
