基于VS CODE 官方文档 [https://code.visualstudio.com/docs/copilot/customization/agent-skills](https://code.visualstudio.com/docs/copilot/customization/agent-skills)
# Agent Skills 机制研究报告
...
```yaml
---
name: skill-name          # 只能字母或-开头，必须和文件夹名相同！否则 silent fail
...
| `.agents/skills/<name>/` | 通用 AI agents ✅（已在 VS Code Chat: Agent Skills Locations 原生支持，2026-04-04 截图确认） |

> ✅ 以上三个路径（及用户级 `~/.copilot/skills` `~/.agents/skills` `~/.claude/skills`）均在 VS Code `chat.skillsLocations` 设置中原生支持，**非幻觉**。

**结论**：V3 的 5 个 Skills 在 `.github/skills/` → Copilot 可用。
对于Claude Code，建相对路径软symlink `.claude/skills`  
